...
select<-grep("mean()|sdt()", names(finaldata6))

finaldata7<-finaldata6[select]
dim(finaldata7)
head(finaldata7)

finaldata8<-cbind(finaldata6$subject,finaldata6$activity,finaldata7)

names(finaldata8)[1]<-"subject"
names(finaldata8)[2]<-"activity"
head(finaldata8)

write.table(finaldata8,"tidydata.txt",row.names=FALSE)
